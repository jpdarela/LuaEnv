...
class LuaPkgConfig:
    ...
    def _show_all_info(self, info: Dict, path_style: str) -> None:
        ...
        print("For MSVC/Visual Studio (cl.exe):")
        print(f"  cl.exe /Fe:myapp.exe myapp.c {cflags} {libs}")
        print()
        print("For MSBuild/Visual Studio projects:")
        print(f"  <AdditionalIncludeDirectories>{norm_include_dir}</AdditionalIncludeDirectories>")
        print(f"  <AdditionalLibraryDirectories>{norm_lib_dir}</AdditionalLibraryDirectories>")
        print("  <AdditionalDependencies>lua54.lib;%(AdditionalDependencies)</AdditionalDependencies>")
        print()
        print("For CMake (MSVC generator):")
        print(f'  target_include_directories(myapp PRIVATE "{norm_include_dir}")')
        print(f'  target_link_directories(myapp PRIVATE "{norm_lib_dir}")')
        print("  target_link_libraries(myapp lua54)")
        print()
